...
from src.aufs.user_tools.fs_meta.ingest_job_data import MainAppWidget, IngestSessionManager
...
class RequestorUI(QWidget):
    ...
    def handle_job_data_ingest(self):
        """Opens the Ingest Job Data popup window."""
        # Create a session manager for the ingest widget
        ingest_session_manager = IngestSessionManager("~/.aufs/config/jobs/active")

        # Initialize the ingest job data widget
        self.ingest_popup = QDialog(self)
        self.ingest_popup.setWindowTitle("Ingest Job Data")
        self.ingest_popup.resize(1024, 768)

        # Embed the MainAppWidget inside the popup
        layout = QVBoxLayout(self.ingest_popup)
        self.ingest_widget = MainAppWidget(session_manager=ingest_session_manager)
        layout.addWidget(self.ingest_widget)

        # Show the popup
        self.ingest_popup.exec()
